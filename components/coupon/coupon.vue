  ...
  <view :class="'coupon-item ' + (canUse ? '' : 'gray')">
    <view class="left">
      ...
      <view class="num" v-if="item.couponType == 2">
        <text class="coupon-price">{{ item.couponDiscount }}</text
        >折
      </view>
    ...
    <view class="right">
      <view class="c-des">
        <text class="c-type">{{
          item.suitableProdType == 0 ? "通用" : "商品"
        }}</text>
        {{ item.suitableProdType == 0 ? "全场通用" : "指定商品可用" }}
      </view>
      <view class="c-date">
        <text
          v-if="showTimeType == 1 && item.couponType == 2"
          class="c-data-info"
          >领券{{ item.validDays }}天后失效</text
        >
        <text v-else class="c-data-info"
